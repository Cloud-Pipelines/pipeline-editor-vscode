...
test("buildArgoWorkflowFromGraphComponent compiles Nested_graph_components", () => {
  const sourcePath = path.resolve(
    __dirname,
    "../testData/Nested_graph_components/pipeline.component.yaml"
  );
  const expectedPath = path.resolve(
    __dirname,
    "./testData/Nested_graph_components/argo_workflow.yaml"
  );
  const pipelineText = fs.readFileSync(sourcePath).toString();
  const pipelineSpec = yaml.load(pipelineText) as ComponentSpec;
  const actualResult = buildArgoWorkflowFromGraphComponent(
